...
@property (nonatomic, strong) UIWindow *connectedKeyWindow;
...
- (void)show
{
    self.connectedKeyWindow = [[UIApplication sharedApplication] keyWindow];
    [self reShow];
}
...
- (void)reShow
{
    ...
    if (!self.alertWindow) {
        CGRect frame =  self.oldKeyWindow.frame;
        UIWindow *window = [[UIWindow alloc] initWithWindowScene:self.connectedKeyWindow.windowScene];
...
- (void)validateLayout
{
    ...
    if (_updateAnimated) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            weak_self.containerView.frame = CGRectMake(left, top, weak_self.containerWidth, height);
            weak_self.blurView.frame = CGRectMake(left, top, weak_self.containerWidth, height);
        }];
    }
    else {
...
- (void)transitionInCompletion:(void(^)(void))completion
{
    ...
    [UIView animateWithDuration:0.3
                     animations:^{
                         if (weak_self.showBlurBackground) {
                             weak_self.containerView.alpha = 1.;
                             weak_self.containerView.backgroundColor = [UIColor clearColor];
                         }else{
                             weak_self.containerView.alpha = weak_self.alertViewAlpha;
                             weak_self.containerView.backgroundColor = weak_self.viewBackgroundColor;
                         }
        weak_self.containerView.transform = CGAffineTransformMakeScale(1.0,1.0);

        weak_self.blurView.alpha = weak_self.alertViewAlpha;
        weak_self.blurView.transform = CGAffineTransformMakeScale(1.0,1.0);
                     }
                     completion:^(BOOL finished) {
                         [weak_self.blurView blur];
                         if (completion) {
                             completion();
                         }
                     }];
...
- (void)transitionOutCompletion:(void(^)(void))completion
{
    ...
    [UIView animateWithDuration:0.25
                     animations:^{
        weak_self.containerView.alpha = 0;
        weak_self.containerView.transform = CGAffineTransformMakeScale(0.9,0.9);

        weak_self.blurView.alpha = 0.9;
        weak_self.blurView.transform = CGAffineTransformMakeScale(0.9,0.9);
                     }
                     completion:^(BOOL finished) {
                         if (completion) {
                             completion();
